...
WINDOWS_CA="example-SERVER-1-CA"
...
yum install -y wget unzip krb5-workstation openldap-clients
yum localinstall -y http://rpmfind.net/linux/centos/6.7/updates/x86_64/Packages/rng-tools-5-2.el6_7.x86_64.rpm
...
if [ ! -d "${CENTRIFY_BASE_DIR}" ]; then
    echo "Centrify base certificate directory does not exist!" >&2
    exit 1;
fi
